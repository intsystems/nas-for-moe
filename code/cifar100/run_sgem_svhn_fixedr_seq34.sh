...
LOG=/home/pbabkin/nas-for-moe/code/cifar100/runs_testsplit/sgem_svhn_fixedr_seq34.log
for S in 3 4; do
  echo "=== start fixedr seed=$S $(date) ===" >> $LOG
  docker exec -e PYTHONUNBUFFERED=1 -e CUDA_VISIBLE_DEVICES=0 nas-moe-container \
    python -u /pbabkin/nas-for-moe/code/cifar100/cifar100_sgem.py \
      --data-dir $DD \
      --save-dir $RT/cifar100_sgem_obs_K2_svhn_pc05_lbw50_fixedr_seed${S} \
      --save-results $RT/results_cifar100_sgem_K2_svhn_pc05_lbw50_fixedr_seed${S}.json \
      --K 2 --phase-c-uniform-mix 0.5 --load-balance-weight 50 --n-r-gradient-steps 0 \
  ...
  echo "=== done fixedr seed=$S $(date) ===" >> $LOG
done
