"""Generate imbalanced toy dataset: linear + ring data with different sample sizes."""
...
def main():
    parser = argparse.ArgumentParser(description="Generate imbalanced toy dataset (linear + ring, different sizes)")
    parser.add_argument("--n-linear", type=int, default=200, help="Number of samples for linear cloud (smaller)")
    parser.add_argument("--n-ring", type=int, default=1800, help="Number of samples for ring cloud (larger)")
    parser.add_argument("--seed", type=int, default=322, help="Random seed")
    parser.add_argument("--output-dir", type=str, default=None,
                        help="Output directory (default: data_imbalanced/ next to script)")
    parser.add_argument("--linear-scale", type=float, default=2.0)
    parser.add_argument("--linear-offset", type=float, default=-5.0)
    parser.add_argument("--inner-radius", type=float, default=1.0)
    parser.add_argument("--outer-radius", type=float, default=2.0)
    parser.add_argument("--ring-noise", type=float, default=0.3)
    ...
    if args.output_dir is None:
        output_dir = Path(__file__).resolve().parent / "data_imbalanced"
    else:
        output_dir = Path(args.output_dir)

    X_linear, y_linear = generate_linear_data(args.n_linear, args.linear_scale, args.linear_offset)
    X_ring, y_ring = generate_ring_data(args.n_ring, args.inner_radius, args.outer_radius, args.ring_noise)
    ...
    print(f"  Linear samples: {args.n_linear}, Ring samples: {args.n_ring}")
    print(f"  Imbalance ratio: 1:{args.n_ring // args.n_linear}")
...
def plot_dataset(X: np.ndarray, y: np.ndarray, data_type: np.ndarray, save_path: Path):
    ...
    ax.scatter(X[circle_mask & (y == 0), 0], X[circle_mask & (y == 0), 1],
               label="Ring, class 0", alpha=0.6, s=50, color="cyan", marker="s")
    ax.scatter(X[circle_mask & (y == 1), 0], X[circle_mask & (y == 1), 1],
               label="Ring, class 1", alpha=0.6, s=50, color="orange", marker="s")

    ax.set_xlabel("x (feature 0)", fontsize=12)
    ax.set_ylabel("y (feature 1)", fontsize=12)
    ax.set_title("Imbalanced dataset (linear: 200, ring: 1800)", fontsize=14)
