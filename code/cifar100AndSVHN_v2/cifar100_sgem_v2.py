...
# --- Пути (v2) ---
SCRIPT_DIR = Path(__file__).resolve().parent  # …/cifar100AndSVHN_v2/
CODE_DIR = SCRIPT_DIR.parent
CIFAR100_DIR = CODE_DIR / "cifar100"
sys.path.insert(0, str(CODE_DIR))
sys.path.insert(0, str(CIFAR100_DIR))  # for cifar100_searchspace, cifar100_final_train
sys.path.insert(0, str(SCRIPT_DIR))    # for utils_v2, optimize_surrogate_em_v2, etc.
...
import optimize_surrogate_em_v2 as osem  # noqa: E402
from optimize_surrogate_em_v2 import optimize_surrogate_em  # noqa: E402
from toy_experiment.optimize_expert_assignments import print_result  # noqa: E402

# v2: override save_observation on collect_dataset to write val_loss
from utils_v2 import save_observation_v2 as _save_observation_v2  # noqa: E402
collect_dataset.save_observation = _save_observation_v2
...
def evaluate_architecture_on_subset_cifar100_v2(
    config: dict,
    search_space,
    b: List[int],
    ...
    """v2: returns mean per-sample CE (val_loss) on the val subset.

    Drop-in replacement for v1 evaluate_architecture_on_subset_cifar100, which
    returned val_acc. The rest of the SGEM pipeline treats the returned scalar
    as the surrogate regression target.

    A side metric `val_acc` is also computed and printed/logged for sanity but
    is NOT used by the surrogate or by ranking decisions in v2.
    """
    X_parts, y_parts = [], []
    ...
    if not X_parts:
        # No data => return a large but finite loss so that q-function / log-lik
        # don't blow up. CE for uniform over C classes is log(C).
        return float(np.log(max(2, _NUM_CLASSES)))
    ...
    val_acc, val_loss = _train_cifar100_net(
        net, X_sub_train, y_sub_train, X_v, y_v,
        epochs=epochs, lr=0.05, batch_size=128, device=device,
    )
    # Side-log for visibility
    print(f"[v2-eval] val_acc={val_acc:.4f} val_loss={val_loss:.4f} "
          f"|b|={int(sum(b))} n_train={len(y_sub_train)} n_val={len(y_v)}")
    return float(val_loss)


# Backward-compatible alias (v1 callers expect this name; v2 routes to loss).
evaluate_architecture_on_subset_cifar100 = evaluate_architecture_on_subset_cifar100_v2
...
def main():
    parser = argparse.ArgumentParser(
        description="Surrogate-EM (SGEM) v2: per-sample CE loss target (CIFAR-100 + SVHN)"
    )
    # --- Данные (подготовлены prepare_cifar100_semantic.py) ---
    parser.add_argument("--data-dir", type=str,
                        default="../cifar100/cifar100_svhn_data_semantic_testsplit",
                        help="Директория с подготовленными данными "
                             "(prepare_cifar100_semantic.py)")
    # --- Результаты ---
    parser.add_argument("--save-dir", type=str,
                        default="./runs_v2/cifar100AndSVHN_v2_sgem_obs")
    parser.add_argument("--save-results", type=str,
                        default="./runs_v2/results_cifar100AndSVHN_v2_sgem.json")
    ...
    parser.add_argument("--per-cluster-eval", action=argparse.BooleanOptionalAction,
                        default=True,
                        help="Оценивать арку на val ТОЛЬКО её кластеров b "
                             "(default: True). --no-per-cluster-eval = global val.")
    ...
    print(f"[main v2] device = {args.device}")
    print(f"[main v2] num_classes={_NUM_CLASSES}, total_samples={len(X)}")
    print(f"[main v2] M={M}, K={args.K}, seed-obs={args.n_seed_observations}, "
    ...
    # v2: compute |C_m| from train_cluster_ids and pass into EM.
    train_cluster_ids = np.load(data_dir / "train_cluster_ids.npy")
    cluster_sizes = np.array(
        [int((train_cluster_ids == m).sum()) for m in range(M)], dtype=np.float32,
    )
    print(f"[main v2] cluster_sizes (train): min={cluster_sizes.min():.0f}, "
          f"max={cluster_sizes.max():.0f}, mean={cluster_sizes.mean():.1f}, "
          f"sum={cluster_sizes.sum():.0f}")

    # Ensure save_dir / save_results parent dirs exist.
