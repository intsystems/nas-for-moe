...
from toy_experiment.collect_dataset import (
    mc_dropout_predict as _mc_dropout_predict,
    config_to_pyg_data as _config_to_pyg_data,
    collate_graphs as _collate_graphs_v2,
)
from torch_geometric.data import Batch as _Batch_v2  # noqa: E402
from torch.utils.data import DataLoader as _DataLoader_v2  # noqa: E402
import random as _random_v2  # noqa: E402
...
from toy_dataset_v2 import ArchSubsetLossDataset


def make_surrogate_loaders_v2(
    observation_paths,
    val_fraction=0.2,
    batch_size=128,
    seed=SEED,
):
    """v2 surrogate loaders: read val_loss instead of val_accuracy.

    Mirrors v1 make_surrogate_loaders but uses ArchSubsetLossDataset.
    """
    n = len(observation_paths)
    indices = list(range(n))
    rng = _random_v2.Random(seed)
    rng.shuffle(indices)
    ...
    train_dataset = ArchSubsetLossDataset(train_paths)
    val_dataset = ArchSubsetLossDataset(val_paths)
    train_loader = _DataLoader_v2(
        train_dataset, batch_size=batch_size, shuffle=True,
        collate_fn=_collate_graphs_v2,
    )
    val_loader = _DataLoader_v2(
...
# v2: shadow the imported make_surrogate_loaders with the v2 one so that
# all retrain_surrogate_from_observations calls use ArchSubsetLossDataset.
make_surrogate_loaders = make_surrogate_loaders_v2


def save_observation(config, b, val_loss, save_dir, index):
    """v2 shim: save observation with val_loss instead of val_accuracy."""
    return save_observation_v2(config, b, val_loss, save_dir, index)


def sample_architectures_for_experts_v2(
    search_space,
    K,
    n_candidates,
    surrogate,
    ...
    """v2 variant: select arch with the LOWEST mean predicted loss (mu).

    Mirrors v1 sample_architectures_for_experts but uses argmin instead of argmax.
    """
    from toy_experiment.optimize_expert_assignments import (
        gumbel_softmax_rows,
        build_graph_data,
    ...
    with torch.no_grad():
        for k in range(K):
            ...
            for _ in range(n_mc_forward):
                R_hard = gumbel_softmax_rows(logits, tau=tau, hard=True)
                R_k = R_hard[:, k]
            ...
            # v2: predicted loss; smaller is better.
            best_idx = int(np.argmin(mean_scores))
            best_configs.append(candidates[best_idx])
    return best_configs


def compute_lcb_score(surrogate, config, b, n_forward=30, device="cpu"):
    """LCB acquisition for loss surrogate: lcb = mu - sigma.

    Lower predicted loss + higher uncertainty => more promising candidate.
    Selection rule downstream uses argmin (lower is better).
    """
    data = _config_to_pyg_data(config, b)
    batch = _Batch_v2.from_data_list([data]).to(device)
    mean_pred, std_pred = _mc_dropout_predict(
...
def refine_architectures_real(
    ...
    for k in range(K):
        ...
        if exhaustive:
            ...
            # v2: val_acc actually holds val_loss; lower is better.
            best_acc = float("inf")
            ...
            for rank, config in enumerate(candidates):
                ...
                if verbose and is_best:
                    print(f"    [{rank+1}/{len(candidates)}] ops={ops} "
                          f"real_val_loss={val_acc:.4f} *** new best")

            best_configs.append(best_config)
            if verbose:
                ops = [best_config.get(f"op_{i}", "?") for i in range(3)]
                print(f"  Expert {k}: best real_val_loss = {best_acc:.4f}, ops={ops}")
        else:
            ...
            # v2: surrogate predicts val_loss; smaller is better.
            top_indices = np.argsort(mean_scores)[:n_top]
            ...
            # Train each top candidate for real, pick best (v2: lowest val_loss).
            best_acc = float("inf")
            ...
            best_configs.append(best_config)
            if verbose:
                print(f"  Expert {k}: best real_val_loss = {best_acc:.4f}")
...
def find_best_archs_from_observations(
    ...
    # v2: score = val_loss; lower is better.
    best_by_b: dict = {}  # b_tuple -> (best_loss, best_config)
    for path in observation_paths:
        try:
            with open(path) as f:
                obs = json.load(f)
            b_tuple = tuple(obs["subset_b"])
            score = read_obs_value(obs)  # val_loss
            if b_tuple not in best_by_b or score < best_by_b[b_tuple][0]:
                best_by_b[b_tuple] = (score, obs["arch"])
...
def collect_s_step_observations(
    ...
    Фаза D: per-expert split-subset exploration (заменяет старую фазу C).
            Для каждого эксперта k берётся его текущая архитектура configs[k]
            и его split (кластеры, где hard_assignments==k). Сэмплируются
            b-вектора — ПОДМНОЖЕСТВА split этого эксперта, размер ~
            Uniform(3, |split_k|-3). Суррогатом (плоское значение, без MC)
            отбираются лучшие (минимальный предсказанный loss), и configs[k]
            реально обучается на них. Бюджет n_explore делится по экспертам.
    ...
    for i in range(n_focused):
        ...
        # v2: LCB over architectures with a fixed b-vector
        # (lcb = mu - sigma; lower predicted loss + uncertainty => more promising).
        ...
        if verbose:
            ...
            print(f"    S-step [B] LCB-focused: clusters={clusters}, "
                  f"lcb={best_score:.4f}, val_loss={val_acc:.4f}")

    # --- Фаза D: per-expert split-subset exploration (заменяет фазу C) ---
    # Для каждого эксперта k: арка configs[k] фиксирована, split_k = кластеры
    # этого эксперта. Сэмплируем b-вектора — ПОДМНОЖЕСТВА split_k, размер ~
    # Uniform(3, |split_k|-3). Суррогатом (плоское значение, без MC) отбираем
    # лучшие (минимальный предсказанный loss) и реально обучаем configs[k].
    # r_soft / phase_c_uniform_mix здесь НЕ используются.
    def _surrogate_value(config, b):
        # Плоская оценка суррогата (dropout off, без MC): ниже = лучше.
        # bool_vec подаётся отдельным тензором [1, M], НЕ как атрибут batch.
        from toy_experiment.optimize_expert_assignments import build_graph_data
        from torch_geometric.data import Data as _PyGData
        surrogate.eval()
        x, edge_index = build_graph_data(config)
        batch = _Batch_v2.from_data_list(
            [_PyGData(x=x, edge_index=edge_index)]).to(device)
        bv = torch.tensor(b, dtype=torch.float, device=device).unsqueeze(0)
        with torch.no_grad():
            out = surrogate(batch.x, batch.edge_index, batch.batch, bv)
        return float(out.squeeze(-1).item())

    def _sample_subset_of(clusters):
        sk = len(clusters)
        if sk <= 3:
            size = sk
        else:
            ...
            size = min(size, sk)
        sel = np.random.choice(clusters, size=size, replace=False)
        b = [0] * n_clusters
        for j in sel:
            b[int(j)] = 1
        return b

    # Бюджет n_explore делим по экспертам.
    per_expert = [n_explore // K + (1 if i < n_explore % K else 0)
                  for i in range(K)]
    ...
    for k in range(K):
        ...
        if n_add <= 0:
            continue
        split_k = [m for m in range(n_clusters) if hard_assignments[m] == k]
        if len(split_k) == 0:
            continue

        # Пул кандидатов-подмножеств → оценка суррогатом → n_add лучших.
        pool_size = max(n_candidates, n_add)
        cand_bs = [_sample_subset_of(split_k) for _ in range(pool_size)]
        if surrogate is not None:
            scored = sorted(
                ((_surrogate_value(configs[k], bb), bb) for bb in cand_bs),
                key=lambda t: t[0],
            )
        else:
            scored = [(float("nan"), bb) for bb in cand_bs]
        chosen = scored[:n_add]

        for pred, b in chosen:
            val_acc = evaluate_architecture_on_subset(
                configs[k], search_space, b,
                X_train_by_cluster, y_train_by_cluster,
                X_val, y_val,
                epochs=cell_train_epochs,
                val_cluster_ids=val_cluster_ids,
            )
            path = save_observation(configs[k], b, val_acc, save_dir, obs_index)
            observation_paths.append(path)
            obs_index += 1
            new_count += 1
            ...
            if verbose:
                clusters = [m for m in range(n_clusters) if b[m] == 1]
                print(f"    S-step [D] expert {k}: |b|={sum(b)}/{len(split_k)}, "
                      f"clusters={clusters[:8]}"
                      f"{'...' if len(clusters) > 8 else ''}, "
                      f"surr={pred:.4f}, val_loss={val_acc:.4f}")
...
def optimize_surrogate_em(
    ...
    # v2: per-cluster sizes |C_m| for size-weighted MoE objective.
    ...
    # v2: cluster sizes |C_m| for size-weighted objective.
    if cluster_sizes is None:
        train_cluster_ids = np.load(Path(cluster_dir) / "train_cluster_ids.npy")
        cluster_sizes = np.array(
            [int((train_cluster_ids == m).sum()) for m in range(n_clusters)],
            dtype=np.float32,
        )
    else:
        cluster_sizes = np.asarray(cluster_sizes, dtype=np.float32)
    cluster_sizes_t = torch.tensor(cluster_sizes, dtype=torch.float, device=device)

    if verbose:
        eval_mode = "per-cluster" if per_cluster_eval else "global"
        print(f"Evaluation mode: {eval_mode}")
        print(f"[v2] cluster_sizes: min={cluster_sizes.min():.0f}, "
              f"max={cluster_sizes.max():.0f}, "
              f"mean={cluster_sizes.mean():.1f}, sum={cluster_sizes.sum():.0f}")
    ...
    for em_iter in iterator:
        ...
        with torch.no_grad():
            ...
            u_vals_np = u_values.cpu().numpy()  # v2: predicted per-sample CE (NLL)

        # v2: q_{mk} \propto r_{mk} * exp(-u_k), normalised over k.
        # Use log-sum-exp for numerical stability.
        log_r = np.log(np.maximum(r_np, 1e-30))
        log_q_unnorm = log_r - u_vals_np[None, :]  # [M, K]
        log_q_unnorm = log_q_unnorm - log_q_unnorm.max(axis=1, keepdims=True)
        q = np.exp(log_q_unnorm)
        q = q / q.sum(axis=1, keepdims=True)
        q_tensor = torch.tensor(q, dtype=torch.float, device=device)

        log_lik = compute_log_likelihood_loss(r_np, u_vals_np, cluster_sizes)
        history.append(log_lik)

        # Snapshot per EM-iter: current configs + hard argmax(r) + n_obs
        iteration_snapshots.append({
            "em_iter": int(em_iter),
            "n_obs": int(len(observation_paths)),
            "configs": copy.deepcopy(configs),
            "hard_assignments": discretize_assignments(r_np).tolist(),
        ...
        for _r_step in range(n_r_gradient_steps):
            ...
            # v2: |C_m|-weighted Σ_m c_m Σ_k q_{mk} · log r_{mk}
            # (deterministic, computed once per step).
            c_w = cluster_sizes_t.unsqueeze(1)  # [M, 1]
            q_log_r = (c_w * q_tensor * log_r).sum()

            # v2: |C_m|-weighted Σ_m c_m Σ_k q_{mk} · (-u_k).
            # u_vals is interpreted as predicted CE (loss), so log-likelihood
            # contribution per sample is -u_vals. MC-average over
            # n_r_mc_samples Gumbel-Softmax (hard) samples of R_k.
            neg_u_acc = torch.zeros(logits.shape[-1], device=device)
            for _ in range(n_r_mc_samples):
                ...
                neg_u_acc = neg_u_acc - u_vals
            neg_u_mean = neg_u_acc / n_r_mc_samples  # [K]

            q_log_u = (c_w * q_tensor * neg_u_mean.unsqueeze(0)).sum()
            ...
            load_balance = K_dim * (P * P).sum()  # normalized, in [1, K]

            # v2 fix (B): the |C_m|-weighted q_function is *extensive*
            # (~Σ|C_m| ≈ 1e4-1e5), but load_balance is normalized (∈[1, K]),
            # so a normalized penalty is ~1e4× too weak — at lbw=0.05 it is a
            # no-op and the collapse wins the q-objective by Δq ≈ 1.6e4.
            # Scale the penalty by the total #samples so both terms share
            # magnitude; lbw then stays O(1) and dataset-size-independent.
            # See experiments-log §5/§6.
            lb_penalty = load_balance * cluster_sizes_t.sum()

            loss = (
                -q_function
                - current_entropy_weight * entropy
                + load_balance_weight * lb_penalty
            )
            loss.backward()
            r_optimizer.step()
            ...
            if verbose and _r_step == n_r_gradient_steps - 1:
                ...
                with torch.no_grad():
                    ...
                    lb_part = load_balance_weight * lb_penalty.item()
                tqdm.write(
                    f"    [M-step scales] -q_function={q_part:.4f} "
                    f"(q_log_r={q_log_r.item():.4f}, q_log_u={q_log_u.item():.4f})  "
                    f"-ew·H={ent_part:.4f}  "
                    f"lbw·LB={lb_part:.4f} "
                    f"(LB_norm={load_balance.item():.4f}, "
                    f"N={int(cluster_sizes_t.sum().item())}, "
                    f"lbw={load_balance_weight:.2f})  "
                    f"loss={loss.item():.4f}"
                )
        ...
        configs = sample_architectures_for_experts_v2(
