"""
Сбор случайного датасета (200 наблюдений) на MNIST и pretraining суррогата.

Этапы:
  1. Загрузка подготовленных MNIST-данных из mnist_data/.
  2. Сэмплирование 200 пар (random arch, random b) + реальное обучение.
  3. Сохранение наблюдений в mnist_random_dataset/obs_*.json.
  4. Обучение суррогата (GAT_Datafeature) на собранном датасете.
  5. Сохранение весов в surr_mnist_pretrained.pth.

Запуск (внутри контейнера):
    docker exec nas-for-moe python \\
        /pbabkin/main/mipt/nas-for-moe/code/toy_experiment/mnist_random_pretrain.py
"""
...
# Патч OPS и MNIST-evaluator импортируются из mnist_sgem.py
import code.toy_experiment.mnist.mnist_sgem as mnist_sgem  # noqa: E402  (делает toy_graph.OPS патч на импорте)
...
def main():
    ...
    parser.add_argument("--save-dir", type=str, default="./runs/mnist_random_dataset")
    parser.add_argument("--checkpoint-path", type=str,
                        default="./runs/surr_mnist_pretrained.pth")
    ...
    # Воспроизводимость + MNIST-evaluator patch
    collect_dataset.set_seed(args.seed)
    mnist_sgem._install_patches()

    # Данные
    data_dir = Path(args.data_dir)
    mnist_sgem.setup_mnist_data(data_dir, n_clusters=20, pca_dim=50, seed=args.seed)
    ...
    ss = mnist_sgem.MNISTDartsSearchSpace(init_channels=args.init_channels)
    ...
    # --- 1. Сбор 200 случайных наблюдений ---
    ...
    for i in pbar:
        ...
        val_acc = mnist_sgem.evaluate_architecture_on_subset_mnist(
    ...
    n_features = len(mnist_sgem.OPS_NEW)
