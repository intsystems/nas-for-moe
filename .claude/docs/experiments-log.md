...
## ⚠️ Смена протокола оценки (2026-05-12)

Введён трёхчастный split **train / val / test** (`prepare_cifar100*.py`,
по умолчанию 0.7/0.15/0.15). Теперь:
- поиск (random 30ep / DARTS supernet / сбор obs SGEM) — train→val;
- финальная модель — обучение на `train ∪ val`, **единственная** оценка на
  отложенном `test` (метрика `test_acc`, ключ в JSON `final_moe`).

**Все числа в таблице ниже — это старый протокол (`val_acc`, retrain на
train→val).** Они НЕ сопоставимы с новыми `test_acc`. После перегенерации
данных и перезапуска прогонов таблицу надо пересобрать на `test_acc`;
старые obs-датасеты несовместимы (cluster IDs изменятся).

## CIFAR-100 + SVHN микс (test_acc, новый протокол, 2026-05-16/17)

Гетерогенный датасет `cifar100_svhn_data_semantic_testsplit` (см.
@docs/cifar100-clustering-variants.md). M=30, **K=2**, semantic clustering,
3 seeds (322/1/2), final-moe retrain 100 эпох на `train∪val` → test.

### Сравнительная таблица (test_acc)

| Метод | n | test_acc (cluster) | test_acc (learnable) |
|---|---|---|---|
| DARTS single (1 net, без MoE/гейта) | 5 | 0.6016 ± 0.011 | — |
| Random-MoE (cluster gate, n200) | 5 | 0.5551 ± 0.031 | — |
| DARTS×K (same arch) | 5 | 0.5913 ± 0.009 | 0.6345 ± 0.007 |
| SGEM `pc05_lbw50` | 5 | 0.5935 ± 0.003 | 0.6291 ± 0.013 |
| SGEM `pc05_lbw30` | 5 | 0.5931 ± 0.006 | 0.6283 ± 0.013 |
| SGEM `pc05_lbw50_fixedr` (ablation 2a) | 5 | 0.5983 ± 0.003 | 0.6409 ± 0.005 |

Поseed'ово (seed 322/1/2):
- DARTS single (5 seeds 322/1/2/3/4): 0.5881/0.5963/0.6056/0.5994/0.6186
- Random-MoE cluster (5 seeds 322/1/2/3/4): 0.5887/0.5244/0.5895/0.5352/0.5376
  (seed1/3/4 — провалы; разброс огромный, std≈0.031)
- DARTS×K (5 seeds 322/1/2/3/4): learn 0.6305/0.6273/0.6444/0.6337/0.6365,
  cluster 0.5808/0.5862/0.5903/0.5940/0.6052
- SGEM lbw50 (5 seeds 322/1/2/3/4): learn 0.6205/0.6456/0.6411/0.6198/0.6186,
  cluster 0.5956/0.5919/0.5941/0.5962/0.5898; hard 10-20/16-14/13-17/17-13/17-13
- SGEM lbw30 (5 seeds 322/1/2/3/4): learn 0.6175/0.6306/0.6457/0.6124/0.6351,
  cluster 0.5983/0.5995/0.5849/0.5917/0.5911
- SGEM lbw50_fixedr (5 seeds 322/1/2/3/4): learn 0.6495/0.6414/0.6387/0.6410/0.6338,
  cluster 0.5949/0.5956/0.5989/0.5986/0.6033

### Ablation 2a: fixed random r (2026-05-17/20, rsm6, завершено)

`cifar100_sgem.py --n-r-gradient-steps 0` — r-матрица фиксируется случайным
hard-разбиением, M-step по r отключён; архитектуры по-прежнему ищутся через
суррогат + active learning. Проверяет вклад обновления routing-матрицы.

- 5 seeds (322/1/2/3/4) готовы (seed3 закончен 2026-05-19 23:58,
  seed4 — 2026-05-20 10:35; логи `runs_testsplit/sgem_svhn_fixedr_seq34.log`).
- cluster test_acc 0.5949/0.5956/0.5989/0.5986/0.6033 → **0.5983 ± 0.003**;
  learnable 0.6495/0.6414/0.6387/0.6410/0.6338 → **0.6409 ± 0.005**.
- fixedr learnable (0.6409) ≈ полный SGEM `pc05_lbw50` (0.6291 ± 0.013),
  fixedr cluster (0.5983) даже чуть выше полного SGEM-cluster (0.5935 ± 0.003).
  Обновление r-матрицы **не даёт прироста** ни по cluster, ни по learnable
  (на 5 seeds fixedr скорее чуть выше полного SGEM по обоим гейтам).
  Подтверждает вывод: преимущество MoE идёт от ансамбля + learnable gate,
  а не от обучаемого cluster-routing'а.

### Ideal split по источнику данных

`meta.json/ideal_split_by_source`: 19 CIFAR-кластеров / 11 SVHN,
**mean purity 0.978** (только c22 смешанный, purity 0.58). Идеальное
разбиение по источнику чётко определено.

SGEM-разбиение **не восстанавливает** этот split (5 seeds, обе серии lbw):
- совпадение hard_assignment с ideal (`max(agree, M−agree)`):
  - `lbw50`: 70.0/56.7/53.3/53.3/66.7% → **mean 60.0%** (splits 10-20/16-14/13-17/17-13/17-13)
  - `lbw30`: 66.7/56.7/53.3/53.3/66.7% → **mean 59.3%** (splits 9-21/14-16/13-17/17-13/17-13)
- база для сбалансированного (~15/15) разбиения против ideal 19/11 ≈ 54–55%
  → SGEM едва выше случайного.
- Виновник — балансовый штраф: ideal несбалансирован (19/11), lbw тянет к 15/15
  → источник размывается. Снижение lbw 50→30 ничего не изменило (59.3% ≈ 60.0%).
- Гипотеза «SGEM выделяет домены данных» **не подтверждается** ни при lbw=50,
  ни при lbw=30.
- Визуализация: `code/cifar100/sgem_svhn_expert_split.png`.

### Выводы (SVHN-микс)

1. **SGEM-learnable (0.6357) ≈ DARTS×K-learnable (0.6341)** —
   неразличимы. Та же картина, что на чистом CIFAR-100: SGEM не
   превосходит «K копий DARTS-архитектуры + learnable gate».
2. **DARTS single (0.5967) — выше всех cluster-MoE.** Одиночная сеть без
   MoE бьёт SGEM-cluster (0.5939), DARTS×K-cluster (0.5858),
   Random-MoE-cluster (0.5675). Hard cluster-разбиение режет данные →
   каждый эксперт видит меньше.
3. Среди cluster-вариантов порядок SGEM > DARTS×K > Random, но SGEM-cluster
   почти не отличим от DARTS single — преимущество идёт **не** от
   source-специализации routing'а (раздел про ideal split выше).
4. **Гипотеза «SGEM выделяет домены» НЕ подтвердилась** — разбиение
   почти случайно относительно границы CIFAR/SVHN и невоспроизводимо.

## Сравнительная таблица: best val_acc (СТАРЫЙ протокол, до 2026-05-12)
